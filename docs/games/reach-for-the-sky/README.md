...
Use `pnpm capture:screenshots` after visible renderer or HUD changes. It captures the desktop/mobile start menu, desktop/mobile opening tower screenshots, the named left/right drawers, daily-report UI, construction ghost, maintenance/transit/value/sentiment/privacy/safety/event lenses, inspection, skyline-victory, public-memory, weather-stress, recovery-contract, invited-visit, and resolved visit-lifecycle states into `test-screenshots/`.
